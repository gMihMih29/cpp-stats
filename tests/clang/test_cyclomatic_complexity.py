...
from cpp_stats.metrics.cyclomatic_complexity import MeanCyclomaticComplexityCalculator, MaxCyclomaticComplexityCalculator
from cpp_stats.metrics.cyclomatic_complexity import MeanCyclomaticComplexityMetric, MaxCyclomaticComplexityMetric, _calculate_cyclomatic_complexity

import clang.cindex

def test_merge_mean_metric_data():
    m1 = MeanCyclomaticComplexityMetric(1, 1)
    m2 = MeanCyclomaticComplexityMetric(10, 1)
    expected_value = 11
    expected_cnt = 2
...
def test_merge_max_metric_data():
    m1 = MaxCyclomaticComplexityMetric(10)
    m2 = MaxCyclomaticComplexityMetric(100)
    expected = 100
...
def test_when_sum_with_unknown_metric_then_not_implemented_1():
    m1 = MeanCyclomaticComplexityMetric(10, 1)
...
def test_calculate_complexity_sum_of_primes(clang_index: clang.cindex.Index):
    ...
    for child in tu.cursor.walk_preorder():
        if (child.kind == clang.cindex.CursorKind.FUNCTION_DECL
            and "test2" in child.displayname):
            func = child
            break
    if func is None:
        assert False
...
def test_happy_path_get_mean_metric():
    m = MeanCyclomaticComplexityMetric(10, 2)
    expected = 5
...
def test_empty_path_get_mean_metric():
    m = MeanCyclomaticComplexityMetric(0, 0)
    expected = 0
...
def test_get_mean_max():
    m = MaxCyclomaticComplexityMetric(5)
    expected = 5
...
def test_get_valid_metric_for_mean(clang_index: clang.cindex.Index):
    tu = clang_index.parse("./tests/data/analyze/cyclomatic_complexity.hpp", args=['-x', 'c++'])
    func = None
    calc = MeanCyclomaticComplexityCalculator()
    for child in tu.cursor.walk_preorder():
...
def test_get_valid_metric_for_meax(clang_index: clang.cindex.Index):
    tu = clang_index.parse("./tests/data/analyze/cyclomatic_complexity.hpp", args=['-x', 'c++'])
    func = None
    calc = MaxCyclomaticComplexityCalculator()
    for child in tu.cursor.walk_preorder():
        if child.kind == clang.cindex.CursorKind.FUNCTION_DECL and child.displayname == "sumOfPrimes(int)":
            func = child
            break
